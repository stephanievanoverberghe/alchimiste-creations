...
import { CalendarClock, FileText, Clock, MessageSquare, ShieldCheck } from 'lucide-react';

export default function CallToActionSection({ note = 'Réponse sous 24–48h ouvrées — devis sans engagement.' }: { note?: string }) {
    return (
        <section id="cta-lancement" aria-labelledby="cta-launch-title" className="relative py-16 md:py-28 px-6 md:px-8 lg:px-[100px] xl:px-[150px]">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 md:gap-6 relative z-[1]">
                        <div>
                            <span className="inline-flex items-center gap-2 text-xs tracking-[0.25em] uppercase text-terracotta bg-terracotta/10 border border-terracotta/30 rounded-full px-4 py-1">
                                ...
                                <span>Prêt à te lancer ?</span>
                            </span>
                            <h2 id="cta-launch-title" className="mt-4 md:mt-6 text-terracotta font-title text-3xl md:text-4xl font-bold tracking-widest leading-tight">
                                On crée un site vivant, à ton image
                            </h2>
                            <p className="mt-3 text-base md:text-lg text-foreground/80 max-w-2xl mx-auto lg:mx-0">
