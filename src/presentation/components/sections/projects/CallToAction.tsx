...
import { CalendarClock, LayoutGrid, ShieldCheck, Clock, UserCheck } from 'lucide-react';

export default function CallToActionProjects({ note = 'Réponse sous 24–48h ouvrées — échange sans pression.' }: { note?: string }) {
    return (
        <section id="cta-projects" aria-labelledby="cta-projects-title" className="relative py-16 md:py-28 px-6 md:px-8 lg:px-[100px] xl:px-[150px]">
                ...
                <div className="group relative overflow-hidden rounded-[22px] border border-sauge/30 bg-background/70 p-6 md:p-10 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
                    ...
                    <div className="mt-6 md:mt-8 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-foreground/80 relative z-[1]">
                        <div className="inline-flex items-center gap-2 rounded-xl border border-sauge/30 bg-sauge/10 px-3 py-2">
                            <Clock className="w-3.5 h-3.5 text-sauge" aria-hidden />1 projet / mois
                        </div>
