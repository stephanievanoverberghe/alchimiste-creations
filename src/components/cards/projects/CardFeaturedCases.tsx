...
export type RawProject = {
    ...
    // citation (plusieurs sources possibles)
    ...
    kind?: string;
    stack?: string;
    status?: string; // ⬅️ élargi à string

    // visuels
    logo?: string; // logo utilisé comme visuel principal
    image?: string;
    media?: { cover?: string };
    ...
    // meta pour la sélection
    featured?: boolean; // ⬅️ ajouté
    priority?: number; // ⬅️ ajouté
    year?: number; // ⬅️ ajouté
};
...
const FALLBACK_LOGO = '/images/placeholders/logo.png'; // change si besoin
...
export default function CardFeaturedCases({ project, className, ctaLabel = 'Voir le projet' }: CardFeaturedCasesProps) {
    ...
    // on privilégie le logo (comme CardProject)
    ...
    return (
        <article
            className={cn(
                'group relative flex h-full flex-col cursor-pointer overflow-hidden rounded-[20px] border border-sauge/30 bg-background shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md pb-4',
                className
            )}
        >
            {/* Media : toujours logo + motif (même principe que CardProject) */}
            <div className="relative aspect-[16/10]">
                ...
                {/* Gradient bas pour la cohérence visuelle */}
...
+                                   text-terracotta/90 hover:text-terracotta transition-colors"
            ...
            {/* Séparateur animé (signature visuelle) */}
