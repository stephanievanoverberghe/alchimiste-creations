// components/method/MethodFinalCTASection.tsx
'use client';

import Link from 'next/link';
import { FileText, CalendarClock, ShieldCheck, Clock, Mail } from 'lucide-react';

export default function CTASection({ note = 'Réponse sous 24–48h ouvrées — devis sans engagement.' }: { primaryHref?: string; secondaryHref?: string; note?: string }) {
    return (
        <section id="contact-final" aria-labelledby="cta-final-title" className="relative py-16 md:py-28 px-6 md:px-8 lg:px-[100px] xl:px-[150px]">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 md:gap-6 relative z-[1]">
                        <div>
                            <span className="inline-flex items-center gap-2 text-xs tracking-[0.25em] uppercase text-terracotta bg-terracotta/10 border border-terracotta/30 rounded-full px-4 py-1">
                                ...
                                <span>On passe à l’action</span>
                            </span>
                            <h2 id="cta-final-title" className="mt-4 md:mt-6 text-terracotta font-title text-3xl md:text-4xl font-bold tracking-widest leading-tight">
                                Prêt à lancer ton projet — avec méthode et sérénité ?
                            </h2>
                            <p className="mt-3 text-base md:text-lg text-foreground/80 max-w-2xl">
                                Tu as tout vu : process, perfs, a11y, livrables. Il reste à poser ton contexte, et on t’envoie une proposition claire.
                            </p>
                        </div>
