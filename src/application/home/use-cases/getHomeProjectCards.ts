import rawProjects from '@/infrastructure/content/projects.json';
import type { CardProjectData } from '@/presentation/components/cards/CardProject';
...
function normalizeProject(project: HomeProjectSource, index: number): CardProjectData {
...
function sortProjectsByDisplayPriority(projects: HomeProjectSource[]): HomeProjectSource[] {
...
export function getHomeProjectCards(projects?: HomeProjectSource[]): CardProjectData[] {
    const source = Array.isArray(projects) && projects.length > 0 ? projects : (rawProjects as HomeProjectSource[]);

    return sortProjectsByDisplayPriority(source).map(normalizeProject).slice(0, 2);
}
